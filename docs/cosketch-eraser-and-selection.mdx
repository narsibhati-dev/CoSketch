...
On a simple drawing app, an eraser might just paint white pixels and selection might be little more than a single highlighted object. CoSketch, however, needs to support:
...
- `apps/cosketch-frontend/src/canvas_engine/eraser.ts`
- `apps/cosketch-frontend/src/canvas_engine/SelectionManager.ts`
...
> [Image placeholder: Diagram of an eraser hitbox overlapping multiple shapes]
...
> [Image placeholder: Diagram showing eraser samples and a freehand path with distance checks]
...
> [Image placeholder: Flowchart of selection states – idle → hover → selected → dragging → idle]
...
> [Image placeholder: Diagram showing one client erasing, WebSocket server relaying updates, and other clients applying them]
