...
> [Image placeholder: High-level Docker architecture diagram – frontend, backend, websocket, database, and network links]
...
> [Image placeholder: Service-level diagram – each Dockerfile producing an image, then services connecting to Postgres]
...
> [Image placeholder: Sequence diagram – client connects to frontend → WebSocket upgrade → collaboration messages flow]
...
> [Image placeholder: Compose topology diagram – services and a shared bridge network]
...
> [Image placeholder: Flow diagram – developer machine vs. production cluster using the same images]
