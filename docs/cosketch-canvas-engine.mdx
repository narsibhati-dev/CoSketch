...
> [Image placeholder: Canvas event flow diagram – pointer events → engine → stores → React components]
...
- [`CanvasEngine.ts`](apps/cosketch-frontend/src/canvas_engine/CanvasEngine.ts): orchestrates tools, shapes, and event handling.
- [`SelectionManager.ts`](apps/cosketch-frontend/src/canvas_engine/SelectionManager.ts): encapsulates logic for tracking and manipulating selected shapes.
- [`eraser.ts`](apps/cosketch-frontend/src/canvas_engine/eraser.ts): provides erasing behavior and hit-testing helpers.
...
- [`tool.store.ts`](apps/cosketch-frontend/src/stores/tool.store.ts)
- [`canvas.store.ts`](apps/cosketch-frontend/src/stores/canvas.store.ts)
- [`canvas_style.store.ts`](apps/cosketch-frontend/src/stores/canvas_style.store.ts)
- [`shape_selected.store.ts`](apps/cosketch-frontend/src/stores/shape_selected.store.ts)
...
> [Image placeholder: State diagram – tool store, canvas store, canvas_style store, and selection store with arrows between them]
...
The React components in `apps/cosketch-frontend/src/components/canvas/` provide the user-facing controls that drive the engine:
...
> [Image placeholder: Component diagram – toolbar/sidebar/footer and a central canvas engine box]
